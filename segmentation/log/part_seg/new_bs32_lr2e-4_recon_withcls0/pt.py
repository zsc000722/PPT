...
class TransformerEncoder(nn.Module):
    def __init__(self, embed_dim=768, depth=4, num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., num_gruop=None):
        super().__init__()
...
class get_model(nn.Module):
    def __init__(self, cls_dim):
        ...
        self.convs1 = nn.Conv1d(4544, 512, 1)
    ...
    def forward(self, pts, cls_label):
        ...
        # final input
        x = torch.cat((cls_tokens, group_input_tokens, group_input_tokens), dim=1)
        pos = torch.cat((cls_pos, pos, pos), dim=1)

        # transformer
        feature_list, cls_feature = self.blocks(x, pos)
        ...
        cls_feature = torch.cat((cls_feature[0], cls_feature[1], cls_feature[2]), dim=2).squeeze().view(B, -1).unsqueeze(-1).repeat(1, 1, N)
        cls_label_one_hot = cls_label.view(B, 16, 1)
        cls_label_feature = self.label_conv(cls_label_one_hot).repeat(1, 1, N)
        x_global_feature = torch.cat((x_max_feature, x_avg_feature, cls_feature, cls_label_feature), 1)  # 1152*2 + 1152cls + 64
