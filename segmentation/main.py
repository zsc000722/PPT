...
def main(args):
    ...
    for name, param in classifier.named_parameters():
        # if 'cls' in name or 'prompt' in name or 'conv' in name or 'pos' in name or 'propagation' in name:
        if 'cls' in name or 'prompt' in name or 'first_conv' in name or 'pos_embed' in name or 'propagation' in name or 'convs' in name or 'bns' in name:
        # if 'conv' in name:
            param.requires_grad = True
            log_string("Trainable params:{}".format(name))
        else:
            param.requires_grad = False

    total_params = sum(p.numel() for p in classifier.parameters())
    trainable_params = sum(p.numel() for p in classifier.parameters() if p.requires_grad)
    log_string("Total params:{:.2f}M, training params:{:.2f}M, {:.2f}%".format(total_params / 1000000,
