...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    if config.model.type == "pos":
        for name, param in base_model.named_parameters():
            if 'cls' in name or 'prompt' in name or 'first_conv' in name or 'pos_embed' in name:
                param.requires_grad = True
                print_log(f"Training params: {name}", logger=logger)
            else:
                param.requires_grad = False

    total_params = sum(p.numel() for p in base_model.parameters())
    trainable_params = sum(p.numel() for p in base_model.parameters() if p.requires_grad)
    print_log("Total params:{:.2f}M, training params:{:.2f}M, {:.2f}%".format(total_params / 1000000,
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            if better:
                ...
                if 'FewShot' not in config.dataset.train._base_.NAME:
        ...
        if epoch > 2 and metrics.acc.item() >= 100: break
    with open(os.path.join(args.experiment_path, "results.txt"), 'a') as result:
        if 'FewShot' in config.dataset.train._base_.NAME:
            best_ac = best_metrics.acc.item()
            result.write('%.2f\t' % (best_ac))
